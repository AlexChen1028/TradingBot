...
# BTC 關鍵區間（2026-06-28 更新）— 58,000 二探止跌、6 萬弱勢無量震盪；空頭主導未變、6 萬=多空生死線，反彈高拋帶再下移 60.5-61.5K
BTC_RESISTANCE_ZONE = (60_500, 61_500)  # 反彈高拋帶（飛揚 6/28：60,500-60,600 死亡三角高空、MA5 壓制 61K；歐陽 6/28：611-615 多頭陷阱套牢區開空）；嚴禁地板追空
BTC_SUPPORT_ZONE    = (58_000, 59_000)  # 破位後短暫支撐/別地板空帶（歐陽 6/26：58-59K 大級別破位後短暫支撐、別地板空＝顺势报复性反彈軋空風險；短期沒支撐、下看前跌區間）；near_support 連動 ≤59,590
BTC_HARD_STOP       = 69_150            # 站上 69,138 大級別關鍵位才轉空（週線 EMA 缺口）
# ETH 關鍵區間（2026-06-27 飛揚更新）— ETH 雙底震盪 1,500-1,620；operative 高空帶上移 1,600-1,620（MA5 1600/M310 1620）、關鍵支撐/雙底 1,500
ETH_RESISTANCE_ZONE = (1_600, 1_620)  # 高空帶（飛揚 6/27：1,600-1,620 壓制＝MA5 1600/M310 1620；突破則上看 1,798 雙底頸線）；1,600 以下為突破多頭區，未到此帶不追空
ETH_SUPPORT_ZONE    = (1_500, 1_520)  # 關鍵支撐帶（飛揚 6/26：1,500-1,510＝2618 關鍵支撐、空單到此跑路）；此區未跌破嚴禁追空（防軋空）
...
SOL_RESISTANCE_ZONE = (70, 72)        # 高空進場帶（飛揚 6/25：SOL 已跌至 64-72 區間、高空帶下移 70-72；M頂雙頂續空）；此區優先放行做空、禁做多
SOL_SUPPORT_ZONE    = (66, 68)        # 止盈/支撐帶（飛揚 6/25：66-68 為支撐、最低打 64.6、傾向跌破 66-68 追空）；地板追空 R:R 差，此區禁空
...
def get_btc_kol_gate(exchange_pub):
    ...
    try:
        ...
        near_support  = bool(btc_px <= BTC_SUPPORT_ZONE[1] * 1.01)  # 追空禁令：接近支撐區（2026-06-16 防守 65.2-65.5K，跌破才有暴跌空間，否則易反彈）
    ...
    for symbol in watch_coins:
        ...
        if result and result['n'] >= min_sig:
            ...
            if d == 1 and SHORT_BIAS and symbol in set(WATCH_ALWAYS) and result['n'] < min_sig + 1:
                ...
                continue
            # ETH 弱勢專屬閘門（2026-06-27 飛揚：ETH 雙底震盪 1,500-1,620，高空帶 1,600-1,620、關鍵支撐 1,500；1,520-1,600 突破多頭區禁追空；破 1,500 放行追空；做多仍僅 1,370-1,390）
            if symbol == 'ETH/USDT:USDT':
                ...
                if d == -1 and ETH_SUPPORT_ZONE[1] < px < ETH_RESISTANCE_ZONE[0]:
                    ...
                    continue
            # SOL 弱勢專屬閘門（2026-06-25 飛揚：SOL 跌至 64-72、高空 70-72、支撐 66-68；弱勢禁多，66-68 地板禁追空）
