...
# BTC 關鍵區間（2026-07-03 更新）— 反彈確認：自 58,000 見底站上 61,100，6 萬關口由壓力/籌碼密集區轉為有效支撐；方向大結構仍偏空（飛揚：反彈中繼、月線需求滿足後下一輪暴跌），目標 62,500-63,000
BTC_RESISTANCE_ZONE = (62_000, 63_000)  # 反彈壓力/目標帶（飛揚 7/3：62,000 以上才做突破追漲多單、不追 61K；歐陽 7/3：62,500-63,000＝第二波反彈目標、破位 70 點站穩才考慮開空）[COSMETIC/未接線]
BTC_SUPPORT_ZONE    = (58_500, 60_000)  # 反彈支撐/回踩接多帶（歐陽 7/2-7/3：6 萬關口轉有效支撐、59,500-60,000 回踩接多止損 59,000；飛揚 7/3：回踩 60,400-60,800 接多；58,500=震盪下沿見底區）；near_support 連動 ≤60,600（BTC 近此帶時禁山寨地板追空、防軋空）
BTC_HARD_STOP       = 69_150            # 站上 69,138 大級別關鍵位才轉空（週線 EMA 缺口）
# ETH 關鍵區間（2026-07-03 更新）— ETH 自 1,500 反彈站上 1,715（匯率隊拉抬領漲）；高空帶上移 1,780-1,800、支撐/回踩接多帶上移 1,600-1,640；1,640-1,780 突破多頭區禁追空（防軋空）
ETH_RESISTANCE_ZONE = (1_780, 1_800)  # 高空帶（歐陽 7/3：1,780 可開空、1,800＝本波頂部/向上空間不足、這波最多再 16 點）；未到此帶不追空
ETH_SUPPORT_ZONE    = (1_600, 1_640)  # 反彈支撐/回踩接多帶（歐陽 7/2-7/3：1,620＝支組復換有效支撐、1,600-1,610 回踩接多；飛揚 7/3：1,675 缺口回踩接多、4H 金三角）；此區未跌破嚴禁追空（防軋空）
...
def get_btc_kol_gate(exchange_pub):
    ...
    try:
        ...
        near_support  = bool(btc_px <= BTC_SUPPORT_ZONE[1] * 1.01)  # 山寨追空禁令：BTC 接近支撐區時（2026-07-03 ≤60,600＝6 萬回踩接多區），BTC 易反彈帶動山寨、防軋空
    ...
    for symbol in watch_coins:
        ...
        if result and result['n'] >= min_sig:
            ...
            if d == 1 and SHORT_BIAS and symbol in set(WATCH_ALWAYS) and result['n'] < min_sig + 1:
                ...
                continue
            # ETH 弱勢專屬閘門（2026-07-03：ETH 反彈站上 1,715，高空帶上移 1,780-1,800、支撐 1,600-1,640；1,640-1,780 突破多頭區禁追空防軋空；破 1,600 放行追空；做多仍僅悲觀二探 1,370-1,390，大結構偏空不追多）
            if symbol == 'ETH/USDT:USDT':
                ...
                if d == -1 and ETH_SUPPORT_ZONE[0] <= px <= ETH_SUPPORT_ZONE[1]:
                    ...
                    continue
                # 做空：突破支撐後的多頭區（1,640-1,780），未到高空帶 1,780-1,800 不追空（2026-07-03 反彈確認、匯率隊領漲，多頭轉強防軋空；歐陽 7/3：到 1,780 才開空）
