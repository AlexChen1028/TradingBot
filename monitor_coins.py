...
SHORT_BIAS            = False  # 2026-07-22 KOL: 龐克明確定性「昨日突破了65,500美元這一線平行高點」（首次用「突破」而非「摸到」，且現貨查證連續多根4h收在65,500上、高探66,924、回踩65,505守住支撐）＝SHORT_BIAS 鬆動門檻（龐克創65,500新高）正式觸發，解除 LONG 額外+1信號限制
COIN_BLACKLIST        = {'CHZ', 'ORDI', 'WLD', 'LAB', 'ADA', 'HYPE', 'BCH', 'BEAT', 'LTC'}  # CHZ: 2026-05-27 BTC歐陽（6/07 復盤利好出盡 0.045→0.035 變廢紙）; ORDI: 2026-06-01 BTC飛揚（廢了）; WLD: 2026-06-02 BTC飛揚（弱勢，禁做多）; LAB: 2026-06-04 BTC飛揚（24→15高位崩盤，勿碰）; ADA: 2026-06-04 BTC飛揚（跌破1.618失活，禁合約）; HYPE: 2026-06-06 BTC飛揚（跌破1.618，思路轉空，禁做多）; BCH: 2026-06-06 BTC飛揚（老牌山寨無操作價值）; BEAT: 2026-06-09 BTC飛揚（0.12→2 主力控盤，空中樓閣隨時崩盤如LAB，嚴禁追高）; LTC: 2026-06-13 加密龐克（老牌弱勢流動性枯竭，關閉抄底網格，禁做多）
...
# BTC 關鍵區間（2026-07-22 更新）— ★★SHORT_BIAS 鬆動門檻觸發：龐克 7/22 明確稱「突破了65,500這一線平行高點」（非摸到），現貨連續多根4h收在65,500上、高探66,924、回踩65,505守住★★；65,500 由壓力翻轉為確認支撐（BTC_SUPPORT_ZONE 上緣不變）；下一關鍵區上移至 68,000-71,000（龐克：短期持有者平均成本線~67,950-68,000／200日均線~71,000，本輪熊市最後最關鍵位置）
BTC_RESISTANCE_ZONE = (68_000, 71_000)  # 下一關鍵壓力帶（龐克 7/22：68,000＝STH平均成本線、71,000＝200日均線，本輪熊市最後最關鍵位；能否帶量上破取決於 CLARITY Act 8/7 前立法進度）[COSMETIC/未接線]
BTC_SUPPORT_ZONE    = (62_000, 63_500)  # 2026-07-28 更新：舊帶 64,000-65,500 已被跌破——歐陽 7/28 早盤稱 63,000-63,500 為新接多試探帶（4h 上行通道下軌）；飛揚 7/28 晚間獨立確認日線大陰線跌破 64,000、最低觸及 63,000、續看 62,000-63,000 下一支撐帶，兩者同日互相印證；near_support 連動 ≤64,135（BTC 回落接多區時禁山寨地板追空、防軋空/合山寨補漲）
BTC_HARD_STOP       = 69_150            # 站上 69,138 大級別關鍵位才轉空（週線 EMA 缺口；已由新 BTC_RESISTANCE_ZONE 68,000-71,000 涵蓋）
# ETH 關鍵區間（2026-07-18 更新）— 1,946 見頂回落跌破 1,850 生死線、現 1,800-1,840 徘徊＝支撐/壓力翻轉：飛揚「1,800-1,820 很強支撐跌破才追空、1,840-1,860 承壓高空、站上 1,860 才反彈開啟」×2、龐克「1,850 收破轉弱→下看 1,592/1,500」、1,905 摸月線 M5＝月線死亡三角警告兌現中；跌破 1,800 後飛揚下個接多看 1,750
ETH_RESISTANCE_ZONE = (1_948, 2_030)  # 高空帶（2026-07-27 更新：飛揚 7/26 晚間標記 1,900 為多空分水嶺、7/27 確認突破並衝高至 1,966；歐陽 7/27 獨立命名 1,948/2,000/2,030 為本輪反彈三次高點所在的強壓區間、建議區間做空不追多）；未到此帶不追空
ETH_SUPPORT_ZONE    = (1_900, 1_930)  # 短線強支撐帶（2026-07-27 更新：舊高空帶 1,840-1,860 已被突破、由壓力翻轉為支撐；飛揚多輪重申 1,900-1,902 為關鍵支撐/分水嶺，7/27 確認突破後現貨仍守在此區之上）；跌破 1,900＝追空啟動重新評估
...
def analyze_major(exchange, symbol):
    ...
    #    ⚠️ 2026-05-27 KOL 修正：FR 已回歸正常水位（~0.01%），不再處於極端負費率狀態
    #       關閉此信號，避免在非嘎空環境下誤判多頭——加密龐克明確要求回歸傳統量價分析
...
_btc_kol_cache = {'fake_breakout': False, 'squeeze_fuel': False, 'near_support': False, 'squeeze_no_short': False, 'ema200': 0, 'fr_raw': 0, 'ts': 0}

# Short-Squeeze Filter 門檻（2026-06-16 加密龐克：極端負費率 + OI 創新高 = 主力惡意軋空起手式）
SQUEEZE_FR_EXTREME = -0.0003   # 資金費率「極端負」門檻（約常態 3 倍）
...
def get_btc_kol_gate(exchange_pub):
    ...
    squeeze_no_short: 極端負費率 + OI 創新高 → 主力惡意軋空，全市場暫停追空（2026-06-16）
    """
    ...
    try:
        ...
        near_support  = bool(btc_px <= BTC_SUPPORT_ZONE[1] * 1.01)  # 山寨追空禁令：BTC 接近支撐區時（2026-07-28 更新 ≤64,135＝62,000-63,500 回踩接多帶），BTC 易反彈帶動山寨補漲、防軋空
        # Short-Squeeze Filter：極端負費率 + OI 創新高 → 主力軋空，暫停所有新追空
        oi_new_high = None  # None=資料不可得（降級為僅看費率）
        try:
            oi_hist = exchange_pub.fetch_open_interest_history('BTC/USDT', '1d', limit=14)
            oi_vals = [float(o.get('openInterestValue') or o.get('openInterestAmount') or 0) for o in oi_hist]
            oi_vals = [v for v in oi_vals if v > 0]
            if len(oi_vals) >= 3:
                oi_new_high = bool(oi_vals[-1] >= max(oi_vals[:-1]))
        except Exception:
            pass
        squeeze_no_short = bool(fr_raw <= SQUEEZE_FR_EXTREME and (oi_new_high is None or oi_new_high))
        _btc_kol_cache.update({
            'fake_breakout': fake_breakout, 'squeeze_fuel': squeeze_fuel,
            'near_support': near_support, 'squeeze_no_short': squeeze_no_short,
            'ema200': ema200, 'fr_raw': fr_raw, 'ts': time.time(),
        })
        label = ('🚀 軋空暫停追空' if squeeze_no_short
                 else '⚠️ 假突破' if fake_breakout
                 else '🔥 嘎空燃料' if squeeze_fuel
                 else '🛑 近支撐追空禁' if near_support
                 else '—')
        oi_tag = '新高' if oi_new_high else ('回落' if oi_new_high is False else 'n/a')
        print(f"  📡 BTC KOL：EMA200={ema200:,.0f}  fr={fr_raw:.5f}  OI={oi_tag}  BTC={btc_px:,.0f}  {label}")
    except Exception as e:
...
def scan_leaderboard(exchange_pub, exchange_priv, candidates, positions, market_bias=0):
    ...
    for symbol, lb_direction, pct in candidates:
        ...
        if lb_direction == 1 and symbol.split('/')[0] in COIN_BLACKLIST:
            print(f"  🚫 {symbol.split('/')[0]} 在黑名單，跳過漲幅榜做多")
            continue
        if lb_direction == 1 and get_btc_kol_gate(exchange_pub).get('fake_breakout'):
            print(f"  ⚠️ KOL: BTC 假突破風險，跳過 {symbol.split('/')[0]} 漲幅榜做多")
            continue
        # SHORT_BIAS 山寨幣：完全停止做多（2026-06-03 KOL）
        if lb_direction == 1 and SHORT_BIAS and symbol not in set(WATCH_ALWAYS):
            print(f"  📉 SHORT_BIAS: {symbol.split('/')[0]} 漲幅榜 LONG 停止，跳過")
            continue
        # SHORT_BIAS 主流幣：需額外 +1 信號
        if lb_direction == 1 and SHORT_BIAS and symbol in set(WATCH_ALWAYS) and result['n'] < MIN_LEADERBOARD_SIGNALS + 1:
            print(f"  📉 SHORT_BIAS: {symbol.split('/')[0]} 漲幅榜 LONG 信號不足（需 {MIN_LEADERBOARD_SIGNALS+1}，有 {result['n']}），跳過")
            continue
        # 2026-06-03 追空禁令：BTC 接近支撐區，山寨幣不追空
        if lb_direction == -1 and symbol not in set(WATCH_ALWAYS) and get_btc_kol_gate(exchange_pub).get('near_support'):
            print(f"  🛑 追空禁令：BTC 接近支撐區，跳過 {symbol.split('/')[0]} 漲幅榜空倉")
            continue
...
def scan(exchange_pub, exchange_priv, watch_coins, positions, market_bias=0):
    ...
    for symbol in watch_coins:
        ...
        if result and result['n'] >= min_sig:
            d = result['direction']
            if d == 1 and symbol.split('/')[0] in COIN_BLACKLIST:
                print(f"  🚫 {symbol.split('/')[0]} 在黑名單，跳過做多")
                continue
            # SHORT_BIAS 山寨幣：完全停止做多（2026-06-03 KOL 共識：完全放棄山寨幣多幻想）
            if d == 1 and SHORT_BIAS and symbol not in set(WATCH_ALWAYS):
                print(f"  📉 SHORT_BIAS: {symbol.split('/')[0]} 山寨幣做多停止，跳過")
                continue
            # SHORT_BIAS 主流幣：需額外 +1 信號
            ...
            if symbol == 'ETH/USDT:USDT':
                ...
                if d == -1 and ETH_SUPPORT_ZONE[1] < px < ETH_RESISTANCE_ZONE[0]:
                    ...
                    continue
            # SOL 弱勢專屬閘門（2026-07-02 飛揚：SOL 自 60 反彈至 80、支撐 60-75、生死線 100-120；弱勢禁追涨，60-75 地板禁追空、逢低接多至 75）
            ...
            if d == 1 and get_btc_kol_gate(exchange_pub).get('fake_breakout'):
                ...
                continue
            # Short-Squeeze Filter（2026-06-16 龐克）：極端負費率 + OI 創新高 → 主力軋空，全市場暫停追空
            if d == -1 and get_btc_kol_gate(exchange_pub).get('squeeze_no_short'):
                print(f"  🚀 軋空風控：極端負費率+OI新高，暫停追空 {symbol.split('/')[0]}")
                continue
            # 追空禁令：BTC 接近支撐區（2026-06-16 防守 65.2-65.5K），山寨幣不追空
