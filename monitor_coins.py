...
# BTC 關鍵區間（2026-06-25 更新）— BTC 破 62K 暴跌至 59K 二探企穩、現 ~61K；新最後支撐/二探＝59-60K（別地板空），反彈目標帶下移 63K
BTC_RESISTANCE_ZONE = (63_000, 64_000)  # 反彈高空帶（歐陽 6/25：超跌反彈至 63,000＝4h 上行通道下軌复换才再跌；飛揚：61-61.5K 今日高空、64K 為 MA5 缺口/前壓）；嚴禁地板追空
BTC_SUPPORT_ZONE    = (59_000, 60_000)  # 二探/最後支撐（歐陽 6/25：放量跌 59K 企穩、60K 关口不能破、60,500 最後做多機會、千万别地板空；飛揚：5萬9）；near_support 追空禁令連動下移至 ≤60,600
BTC_HARD_STOP       = 69_150            # 站上 69,138 大級別關鍵位才轉空（週線 EMA 缺口）
# ETH 關鍵區間（2026-06-24 晚間飛揚更新）— ETH 突破失敗、跌回 1,604-1,692 區間，operative 壓制下移至 1,670-1,690（連兩日精準承壓、今 1,692 拒絕）；破 1,600 看更大跌幅
ETH_RESISTANCE_ZONE = (1_670, 1_720)  # 高空帶（飛揚 6/24：1,670-1,690 反彈承壓帶、連兩日驗證；1,800 為更高上緣若強彈）；1,670 以下為突破多頭區，未到此帶不追空
    ...
    for symbol in watch_coins:
        ...
        if result and result['n'] >= min_sig:
            ...
            if d == 1 and SHORT_BIAS and symbol in set(WATCH_ALWAYS) and result['n'] < min_sig + 1:
                ...
                continue
            # ETH 弱勢專屬閘門（2026-06-24 晚間飛揚：ETH 突破失敗、operative 壓制下移 1,670-1,690，高空帶 1,670-1,720；1,640-1,670 突破多頭區禁追空；破 1,600 放行追空；做多仍僅 1,370-1,390）
